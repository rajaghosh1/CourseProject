...
======
	...
	+ This will process the datasets and create a tidy data file
	  TidyData.txt in the current directory

(See the CodeBook.md for a description of fields for the tidy data)

Alternately you can call run_analysis(<directory>) with the directory
containing the UCI HAR Dataset

If you want to get the tidy_data witin R (without dumping to the file),
call either one of the following
	+ tidy_data <- get_tidy_data() # from dataset directory
	+ tidy_data <- get_tidy_data(<dataset directory>)